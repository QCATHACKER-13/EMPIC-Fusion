...
R =  0.0821 #L·atm/(mol·K),8.314 J/(mol·K), ideal gas constant
...
R1 = np.sqrt(np.power(0.0802,2) - np.power(0.038,2))#m, final radii of the electromagnet and initial radii of the electromagnet
...
vac_pressure = 1 #atm (standard atmospheric pressure), 760 mmHg
...
x1, y1, z1 = np.meshgrid(np.linspace(-R1, R1, ds),np.linspace(-R1, R1, ds),np.linspace(0, l2, ds))
...
def H(m, q, k, A, V):
    H = (m*np.power(c0,2)) - (((np.power(h_bar,2))/(2*m))*(np.power(k[0],2) + np.power(k[1],2) + np.power(k[2],2))) + (((q*h_bar)/(2*m))*(()+()))
...
Bx1_2, By1_2, Bz1_2 = B2(I,[0, 0, 0], x1, y1, z1)
Bx2_2, By2_2, Bz2_2 = B2(I,[0, 0, l2], x1, y1, z1)
Bx2 = Bx1_2 - Bx2_2
By2 = By1_2 - By2_2
Bz2 = Bz1_2 - Bz2_2
