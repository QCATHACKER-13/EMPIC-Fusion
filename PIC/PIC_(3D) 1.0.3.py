...
x_velocity = np.random.uniform(-c0, c0, num_particles)
y_velocity = np.random.uniform(-c0, c0, num_particles)
z_velocity = np.random.uniform(-c0, c0, num_particles)
...
for step in range(1000):
    ...
    for particle in range(num_particles):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 <= grid_z < Nz):
            ...
            x_acceleration = ((e/(mp))*(Ex_field[grid_x] + ((((y_velocity[particle]*(B1z_field[grid_z] + B2z_field[grid_z])) - ((z_velocity[particle]*(B1y_field[grid_y] + B2y_field[grid_y]))))))))
            y_acceleration = ((e/(mp))*(Ey_field[grid_y] + ((((z_velocity[particle]*(B1x_field[grid_x] + B2x_field[grid_x])) - ((x_velocity[particle]*(B1z_field[grid_z] + B2y_field[grid_z]))))))))
            z_acceleration = ((e/(mp))*(Ex_field[grid_z] + ((((x_velocity[particle]*(B1y_field[grid_y] + B2y_field[grid_y])) - ((y_velocity[particle]*(B1x_field[grid_x] + B2x_field[grid_x]))))))))
    ...
    print(f"({step}) steps: {x_velocity[:],y_velocity[:],z_velocity[:]}:")
