...
for step in range(steps):
    ...
    for Nei in range(num_particles[0]):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 < grid_z <= Nz):
            xe_final, ye_final, ze_final, vx_e_final, vy_e_final, vz_e_final = electron([Nei,0],
            ...
            #velocity of the particle in electrostatic force between electrode surface area and charged particle
            vx_electron[Nei] += vx_e_final
            vy_electron[Nei] += vy_e_final
            vz_electron[Nei] += vz_e_final

            #final position of the particle
            x_electron[Nei] += xe_final
            y_electron[Nei] += ye_final
            z_electron[Nei] += ze_final
    ...
    for Npi in range(num_particles[1]):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 < grid_z <= Nz):
            xp_final, yp_final, zp_final, vx_p_final, vy_p_final, vz_p_final = proton([Npi,0],
            ...
            #velocity of the particle in electrostatic force between electrode surface area and charged particle
            vx_proton[Npi] += vx_p_final
            vy_proton[Npi] += vy_p_final
            vz_proton[Npi] += vz_p_final

            #final position of the particle
            x_proton[Npi] += xp_final
            y_proton[Npi] += yp_final
            z_proton[Npi] += zp_final
            ...
            for Nej in range(num_particles[0]):
                ...
                if (0 <= grid_xje < Nx) and (0 <= grid_yje < Ny) and (0 < grid_zje <= Nz):
                    ...
                    if not(Nei == Nej):
                        ...
                        xe1_final, ye1_final, ze1_final, vx_e1_final, vy_e1_final, vz_e1_final = electron([Nei,Nej],
                                       [vx_electron[Nei], vy_electron[Nei], vz_electron[Nei]],
                                       [Ex_field[0][grid_xie], Ey_field[0][grid_yie], Ez_field[0][grid_zie]],
                                       [Bx_field[0][grid_xie], By_field[0][grid_xie], Bz_field[0][grid_yie]]
                                        )
                        xe2_final, ye2_final, ze2_final, vx_e2_final, vy_e2_final, vz_e2_final = electron([Nej,Nei],
            ...
            for Npj in range(num_particles[0]):
                ...
                if (0 <= grid_xjp < Nx) and (0 <= grid_yjp < Ny) and (0 < grid_zjp <= Nz):
                    ...
                    if not(Npi == Npj):
                        ...
                        xp1_final, yp1_final, zp1_final, vx_p1_final, vy_p1_final, vz_p1_final = proton([Npi,Npj],
                                       [vx_proton[Npi], vy_proton[Npi], vz_proton[Npi]],
                                       [Ex_field[1][grid_xip], Ey_field[1][grid_yip], Ez_field[1][grid_zip]],
                                       [Bx_field[1][grid_xip], By_field[1][grid_xip], Bz_field[1][grid_yip]]
                                        )
                        xp2_final, yp2_final, zp2_final, vx_p2_final, vy_p2_final, vz_p2_final = proton([Npj,Npi],
        ...
        if (0 <= grid_xe < Nx) and (0 <= grid_ye < Ny) and (0 < grid_ze <= Nz):
            ...
            for Np in range(num_particles[1]):
                ...
                if (0 <= grid_xp < Nx) and (0 <= grid_yp < Ny) and (0 < grid_zp <= Nz):
                    ...
                    xe_final, ye_final, ze_final, vx_e_final, vy_e_final, vz_e_final = electron([Ne,Np],
                    ...
                    xp_final, yp_final, zp_final, vx_p_final, vy_p_final, vz_p_final = proton([Np,Ne],
