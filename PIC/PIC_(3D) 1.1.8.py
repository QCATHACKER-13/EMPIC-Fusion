...
for step in range(steps):
    ...
    for Nei in range(num_particles[0]):
        ...
        x = (x_electron[Nei] + ((l1 - 2*a)/(b * Nx)))
        y = (y_electron[Nei] + ((2*b)/Ny))
        z = (z_electron[Nei] + ((2*b)/Ny))
        ...
        Ex_field[0][Nei] = (Ex1 + Ex2)
        Ey_field[0][Nei] = (Ey1 + Ey2)
        Ez_field[0][Nei] = (Ez1 + Ez2)
        ...
        Bx_field[0][Nei] = (0.5*((B1_x1 + B1_x2) + (B2_x1 + B2_x2)))
        By_field[0][Nei] = (0.5*((B1_y1 + B1_y2) + (B2_y1 + B2_y2)))
        Bz_field[0][Nei] = (0.5*((B1_z1 + B1_z2) + (B2_z1 + B2_z2)))

        ve_x[Nei] = ((-e/me)*(Ex1 + Ex2)*dt)
        ve_y[Nei] = ((-e/me)*(Ey1 + Ex2)*dt)
        ve_z[Nei] = ((-e/me)*(Ez1 + Ex2)*dt)
    ...
    for Nei in range(num_particles[0]):
        ...
        electron_particle = Particle(me, -e, [ve_x[Nei], ve_y[Nei], ve_z[Nei]],
                                     [Ex_field[0][Nei], Ey_field[0][Nei], Ez_field[0][Nei]],
                                     [Bx_field[0][Nei], By_field[0][Nei], Bz_field[0][Nei]],
                                     dt)
    ...
    for Npi in range(num_particles[1]):
        ...
        x = (x_proton[Npi] + ((l1 - 2*a)/(b * Nx)))
        y = (y_proton[Npi] + ((2*b)/Ny))
        z = (z_proton[Npi] + ((2*b)/Ny))
        ...
        Bx_field[1][Npi] = (0.5*((B1_x1 + B1_x2) + (B2_x1 + B2_x2)))
        By_field[1][Npi] = (0.5*((B1_y1 + B1_y2) + (B2_y1 + B2_y2)))
        Bz_field[1][Npi] = (0.5*((B1_z1 + B1_z2) + (B2_z1 + B2_z2)))

        vp_x[Npi] = ((-e/me)*(Ex1 + Ex2)*dt)
    ...
    for Npi in range(num_particles[1]):
        ...
        proton_particle = Particle(mp, e, [vp_x[Npi], vp_y[Npi], vp_z[Npi]],
                                   [Ex_field[1][Npi], Ey_field[1][Npi], Ez_field[1][Npi]],
                                   [Bx_field[1][Npi], By_field[1][Npi], Bz_field[1][Npi]],
                                   dt)
    ...
    for Nei in range(num_particles[0]):
        ...
        #Electric Field between Electron and Electrode
        S1a_electron_field = Field([0, 0, a], [x1, y1, z1])
        S1b_electron_field = Field([0, 0, l1 - a], [x1, y1, z1])
        ...
        Bxie1_1, Byie1_1, Bzie1_1 = S1a_electron_field.B1(I)
        Bxie2_1, Byie2_1, Bzie2_1 = S1b_electron_field.B1(I)

        Bxie1_2, Byie1_2, Bzie1_2 = S1a_electron_field.B2(I)
        Bxie2_2, Byie2_2, Bzie2_2 = S1b_electron_field.B2(I)
    ...
    for Ne in range(num_particles[0]):
        ...
        ve_x[Ne] = (0.5*(np.sqrt((2*np.abs(Uqex2))/me) + np.sqrt((2*np.abs(Uqex1))/me)))
        ve_y[Ne] = (0.5*(np.sqrt((2*np.abs(Uqey2))/me) + np.sqrt((2*np.abs(Uqey1))/me)))
        ve_z[Ne] = (0.5*(np.sqrt((2*np.abs(Uqez2))/me) + np.sqrt((2*np.abs(Uqez1))/me)))
        ...
        for Np in range(num_particles[1]):
            ...
            vp_x[Np] = (0.5*(np.sqrt((2*np.abs(Uqpx1))/mp) + np.sqrt((2*np.abs(Uqpx2))/mp)))
            vp_y[Np] = (0.5*(np.sqrt((2*np.abs(Uqpy1))/mp) + np.sqrt((2*np.abs(Uqpy2))/mp)))
            vp_z[Np] = (0.5*(np.sqrt((2*np.abs(Uqpz1))/mp) + np.sqrt((2*np.abs(Uqpz2))/mp)))
