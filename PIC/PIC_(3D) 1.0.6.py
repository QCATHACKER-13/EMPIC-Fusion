...
def U(q1, q2, a7, X, Y, Z):
    ...
    U_z = ko*q1*q2*((Z - a7[2])/np.power(r,2))
    return U_x, U_y, U_z
...
for step in range(steps):
    ...
    for Nei in range(num_particles[0]):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 < grid_z <= Nz):
            ...
            x_acceleration = (-e/me)*(Ex_field[0][grid_x] + (0.5*vx_electron[Nei]*(B1x_field[0][grid_x] + B2x_field[0][grid_x])))
            y_acceleration = (-e/me)*(Ey_field[0][grid_y] + (0.5*vy_electron[Nei]*(B1y_field[0][grid_y] + B2y_field[0][grid_y])))
            z_acceleration = (-e/me)*(Ez_field[0][grid_z] + (0.5*vz_electron[Nei]*(B1z_field[0][grid_z] + B2z_field[0][grid_z])))

            #velocity of the particle in electrostatic force between electrode surface area and charged particle
            vx_electron[Nei] += (0.5*x_acceleration*dt)/np.sqrt(1 - np.power((x_acceleration*dt)/c0,2))
            vy_electron[Nei] += (0.5*y_acceleration*dt)/np.sqrt(1 - np.power((y_acceleration*dt)/c0,2))
            vz_electron[Nei] += (0.5*z_acceleration*dt)/np.sqrt(1 - np.power((z_acceleration*dt)/c0,2))

            #final position of the particle
            x_electron[Nei] += (vx_electron[Nei]*dt)/np.sqrt(1 - np.power((x_acceleration*dt)/c0,2))
            y_electron[Nei] += (vy_electron[Nei]*dt)/np.sqrt(1 - np.power((y_acceleration*dt)/c0,2))
            z_electron[Nei] += (vz_electron[Nei]*dt)/np.sqrt(1 - np.power((z_acceleration*dt)/c0,2))
    ...
    for Npi in range(num_particles[1]):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 < grid_z <= Nz):
            ...
            B1y_field[1][grid_y] += By1
            B2y_field[1][grid_y] += By2
    ...
    for Npi in range(num_particles[1]):
        ...
        if (0 <= grid_x < Nx) and (0 <= grid_y < Ny) and (0 < grid_z <= Nz):
            #acceleration of the particle in electrostatic force between electrode surface area and charged particle
            x_acceleration = (e/mp)*(Ex_field[1][grid_x] + (0.5*vx_proton[Npi]*(B1x_field[1][grid_x] + B2x_field[1][grid_x])))
            y_acceleration = (e/mp)*(Ey_field[1][grid_y] + (0.5*vy_proton[Npi]*(B1y_field[1][grid_y] + B2y_field[1][grid_y])))
            z_acceleration = (e/mp)*(Ez_field[1][grid_z] + (0.5*vz_proton[Npi]*(B1z_field[1][grid_z] + B2z_field[1][grid_z])))

            #velocity of the particle in electrostatic force between electrode surface area and charged particle
            vx_proton[Npi] += (0.5*x_acceleration*dt)/np.sqrt(1 - np.power((x_acceleration*dt)/c0,2))
            vy_proton[Npi] += (0.5*y_acceleration*dt)/np.sqrt(1 - np.power((y_acceleration*dt)/c0,2))
            vz_proton[Npi] += (0.5*z_acceleration*dt)/np.sqrt(1 - np.power((z_acceleration*dt)/c0,2))

            #final position of the particle
            x_proton[num_proton] += (vx_proton[Npi]*dt)/np.sqrt(1 - np.power((x_acceleration*dt)/c0,2))
            y_proton[num_proton] += (vy_proton[Npi]*dt)/np.sqrt(1 - np.power((y_acceleration*dt)/c0,2))
            z_proton[num_proton] += (vz_proton[Npi]*dt)/np.sqrt(1 - np.power((z_acceleration*dt)/c0,2))
    #-----------------------------Particle - Field Interaction -----------------------------
    #----------------------------------------END--------------------------------------------
        ...
        if (0 <= grid_xie < Nx) and (0 <= grid_yie < Ny) and (0 < grid_zie <= Nz):
            ...
            for Nej in range(num_particles[0]):
                ...
                if (0 <= grid_xje < Nx) and (0 <= grid_yje < Ny) and (0 < grid_zje <= Nz):
                    ...
                    xi_acceleration = (-e/me)*(Ex_field[0][grid_xie] + (0.5*vx_electron[Nei]*(B1x_field[0][grid_xie] + B2x_field[0][grid_xie])))
                    yi_acceleration = (-e/me)*(Ey_field[0][grid_yie] + (0.5*vy_electron[Nei]*(B1y_field[0][grid_yie] + B2y_field[0][grid_yie])))
                    zi_acceleration = (-e/me)*(Ez_field[0][grid_zie] + (0.5*vz_electron[Nei]*(B1z_field[0][grid_zie] + B2z_field[0][grid_zie])))

                    xj_acceleration(-e/me)*(Ex_field[0][grid_xje] + (0.5*vx_electron[Nej]*(B1x_field[0][grid_xje] + B2x_field[0][grid_xje])))
                    yj_acceleration = (-e/me)*(Ey_field[0][grid_yje] + (0.5*vy_electron[Nej]*(B1y_field[0][grid_yje] + B2y_field[0][grid_yje])))
                    zj_acceleration = (-e/me)*(Ez_field[0][grid_zje] + (0.5*vz_electron[Nej]*(B1z_field[0][grid_zje] + B2z_field[0][grid_zje])))

                    if not(Nei == Nej):
                        #velocity of the electron in electrostatic force between electron particles
                        vx_electron[Nei] += (0.5*(xi_acceleration + xj_acceleration)*dt)/np.sqrt(1 - np.power(((xi_acceleration + xj_acceleration)*dt)/c0,2))
                        vy_electron[Nei] += (0.5*(yi_acceleration + yj_acceleration)*dt)/np.sqrt(1 - np.power(((yi_acceleration + yj_acceleration)*dt)/c0,2))
                        vz_electron[Nei] += (0.5*(zi_acceleration + zj_acceleration)*dt)/np.sqrt(1 - np.power(((zi_acceleration + zj_acceleration)*dt)/c0,2))

                        #final position of the electrons
                        x_electron[Nei] += (0.5*(vx_electron[Nei] + vx_electron[Nej])*dt)/np.sqrt(1 - np.power(((xi_acceleration + xj_acceleration)*dt)/c0,2))
                        y_electron[Nei] += (0.5*(vx_electron[Nei] + vx_electron[Nej])*dt)/np.sqrt(1 - np.power(((yi_acceleration + yj_acceleration)*dt)/c0,2))
                        z_electron[Nei] += (0.5*(vx_electron[Nei] + vx_electron[Nej])*dt)/np.sqrt(1 - np.power(((zi_acceleration + zj_acceleration)*dt)/c0,2))
    ...
    for Npi in range(num_particles[1]):
        ...
        if (0 <= grid_xip < Nx) and (0 <= grid_yip < Ny) and (0 < grid_zip <= Nz):
            ...
            for Npj in range(num_particles[0]):
                xjp = x_electron[Npj]/(b * Nx)
                yjp = y_electron[Npj]/(b * Ny)
                zjp = z_electron[Npj]/(l1 * Nz)
                ...
                if (0 <= grid_xjp < Nx) and (0 <= grid_yjp < Ny) and (0 < grid_zjp <= Nz):
                    ...
                    xi_acceleration = (e/mp)*(Ex_field[1][grid_xi_p] + (0.5*vx_proton[Npi]*(B1x_field[1][grid_xi_p] + B2x_field[1][grid_xi_p])))
                    yi_acceleration = (e/mp)*(Ey_field[1][grid_yi_p] + (0.5*vy_proton[Npi]*(B1y_field[1][grid_yi_p] + B2y_field[1][grid_yi_p])))
                    zi_acceleration = (e/mp)*(Ez_field[1][grid_zi_p] + (0.5*vz_proton[Npi]*(B1z_field[1][grid_zi_p] + B2z_field[1][grid_zi_p])))

                    xj_acceleration = (e/mp)*(Ex_field[1][grid_xjp] + (0.5*vx_proton[Npj]*(B1x_field[1][grid_xjp] + B2x_field[1][grid_xjp])))
                    yj_acceleration = (e/mp)*(Ey_field[1][grid_yjp] + (0.5*vy_proton[Npj]*(B1y_field[1][grid_yjp] + B2y_field[1][grid_yjp])))
                    zj_acceleration = (e/mp)*(Ez_field[1][grid_zjp] + (0.5*vz_proton[Npj]*(B1z_field[1][grid_zjp] + B2z_field[1][grid_zjp])))

                    if not(Npi == Npj):
                        
                        #velocity of the proton in electrostatic force between proton particles
                        vx_proton[Npi] += (0.5*(xi_acceleration + xj_acceleration)*dt)/np.sqrt(1 - np.power(((xi_acceleration + xj_acceleration)*dt)/c0,2))
                        vy_proton[Npi] += (0.5*(yi_acceleration + yj_acceleration)*dt)/np.sqrt(1 - np.power(((yi_acceleration + yj_acceleration)*dt)/c0,2))
                        vz_proton[Npi] += (0.5*(zi_acceleration + zj_acceleration)*dt)/np.sqrt(1 - np.power(((zi_acceleration + zj_acceleration)*dt)/c0,2))

                        #final position of the protons
                        x_proton[Npi] += (0.5*(vx_proton[Npi] + vx_proton[Npj])*dt)/np.sqrt(1 - np.power(((xi_acceleration + xj_acceleration)*dt)/c0,2))
                        y_proton[Npi] += (0.5*(vy_proton[Npi] + vy_proton[Npj])*dt)/np.sqrt(1 - np.power(((yi_acceleration + yj_acceleration)*dt)/c0,2))
                        z_proton[Npi] += (0.5*(vz_proton[Npi] + vz_proton[Npj])*dt)/np.sqrt(1 - np.power(((zi_acceleration + zj_acceleration)*dt)/c0,2))
        ...
        if (0 <= grid_xe < Nx) and (0 <= grid_ye < Ny) and (0 < grid_ze <= Nz):
            ...
            for Np in range(num_particles[1]):
                ...
                if (0 <= grid_xp < Nx) and (0 <= grid_yp < Ny) and (0 < grid_zp <= Nz):
                    ...
                    #acceleration of the particle in electrostatic force between electrode surface area and charged particle
                    xe_acceleration = (-e/me)*(Ex_field[0][grid_xe] + (0.5*vx_electron[Ne]*(B1x_field[0][grid_xe] + B2x_field[0][grid_xe])))
                    ye_acceleration = (-e/me)*(Ey_field[0][grid_ye] + (0.5*vy_electron[Ne]*(B1y_field[0][grid_ye] + B2y_field[0][grid_ye])))
                    ze_acceleration = (-e/me)*(Ez_field[0][grid_ze] + (0.5*vz_electron[Ne]*(B1z_field[0][grid_ze] + B2z_field[0][grid_ze])))

                    xj_acceleration = (e/mp)*(Ex_field[1][grid_xp] + (0.5*vx_proton[Np]*(B1x_field[1][grid_xp] + B2x_field[1][grid_xp])))
                    yj_acceleration = (e/mp)*(Ey_field[1][grid_yp] + (0.5*vy_proton[Np]*(B1y_field[1][grid_yp] + B2y_field[1][grid_yp])))
                    zj_acceleration = (e/mp)*(Ez_field[1][grid_zp] + (0.5*vz_proton[Np]*(B1z_field[1][grid_zp] + B2z_field[1][grid_zp])))
                    
                    #velocity of the elctron in electrostatic force between electron and proton
                    vx_electron[Ne] += (0.5*(xp_acceleration + xe_acceleration)*dt)/np.sqrt(1 - np.power(((xp_acceleration + xe_acceleration)*dt)/c0,2))
                    vy_electron[Ne] += (0.5*(yp_acceleration + ye_acceleration)*dt)/np.sqrt(1 - np.power(((yp_acceleration + ye_acceleration)*dt)/c0,2))
                    vz_electron[Ne] += (0.5*(zp_acceleration + ze_acceleration)*dt)/np.sqrt(1 - np.power(((zp_acceleration + ze_acceleration)*dt)/c0,2))
                        
                    #velocity of the proton in electrostatic force between electron and proton
                    vx_proton[Np] += (0.5*(xp_acceleration + xe_acceleration)*dt)/np.sqrt(1 - np.power(((xp_acceleration + xe_acceleration)*dt)/c0,2))
                    vy_proton[Np] += (0.5*(yp_acceleration + ye_acceleration)*dt)/np.sqrt(1 - np.power(((yp_acceleration + ye_acceleration)*dt)/c0,2))
                    vz_proton[Np] += (0.5*(zp_acceleration + ze_acceleration)*dt)/np.sqrt(1 - np.power(((zp_acceleration + ze_acceleration)*dt)/c0,2))

                    #final position of the electrons
                    x_electron[Ne] += (0.5*(vx_proton[Np] + vx_electron[Ne])*dt)/np.sqrt(1 - np.power(((xp_acceleration + xe_acceleration)*dt)/c0,2))
                    y_electron[Ne] += (0.5*(vy_proton[Np] + vy_electron[Ne])*dt)/np.sqrt(1 - np.power(((yp_acceleration + ye_acceleration)*dt)/c0,2))
                    z_electron[Ne] += (0.5*(vz_proton[Np] + vz_electron[Ne])*dt)/np.sqrt(1 - np.power(((zp_acceleration + ze_acceleration)*dt)/c0,2))
                    
                    #final position of the proton
                    x_proton[Np] += (0.5*(vx_proton[Np] + vx_electron[Ne])*dt)/np.sqrt(1 - np.power(((xp_acceleration + xe_acceleration)*dt)/c0,2))
                    y_proton[Np] += (0.5*(vy_proton[Np] + vy_electron[Ne])*dt)/np.sqrt(1 - np.power(((yp_acceleration + ye_acceleration)*dt)/c0,2))
                    z_proton[Np] += (0.5*(vz_proton[Np] + vz_electron[Ne])*dt)/np.sqrt(1 - np.power(((zp_acceleration + ze_acceleration)*dt)/c0,2))
