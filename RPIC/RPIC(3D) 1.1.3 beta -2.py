...
r0 = 1.2e-15 # nuclear radius parameter
...
class Quant_Prob:
    ...
    def prob_collision(self, v1, v2, U):
        v_r = np.sqrt(np.power(v2[0] - v1[0],2) + np.power(v2[1] - v1[1],2) + np.power(v2[2] - v1[2],2))
        vx_cm = (self.m1*v1[0] + self.m2*v2[0])/(self.m1 + self.m2)
        vy_cm = (self.m1*v1[1] + self.m2*v2[1])/(self.m1 + self.m2)
        vz_cm = (self.m1*v1[2] + self.m2*v2[2])/(self.m1 + self.m2)
        ...
        deBroglie_lambda = (2*np.pi*self.h_bar)/np.sqrt(2*np.pi*self.reduce_mass*U)

        #Coulumb logarithm
        Coulumb_log = np.log(12*np.pi*self.density_H*deBroglie_lambda)

        col_csec = (4*self.k0*(self.Z1*self.Z2)*np.power(self.e, 4)*Coulumb_log)/(np.power(self.mu, 2)*np.power(v_cm, 4))

        prob_col = self.density_H*v_cm*col_csec

        return col_csec, prob_col
    def Max_Boltz(self, n1, n2, x1, x2, F_prob, C_counts, U):
        ...
        #Coulumb logarithm
        Coulumb_log = np.log(12*np.pi*self.denisty_H*deBroglie_lambda)

        #Maxwell-Boltzmann Distribution
        dis_x = (4*Coulumb_log*F_prob*np.power(C_counts/(self.density_H*(dx/np.power(r, 2))), 3))
        dis_y = (4*Coulumb_log*F_prob*np.power(C_counts/(self.density_H*(dy/np.power(r, 2))), 3))
        dis_z = (4*Coulumb_log*F_prob*np.power(C_counts/(self.density_H*(dz/np.power(r, 2))), 3))
        return deBroglie_lambda, Coulumb_log, dis_x, dis_y, dis_z
...
print("Step    Proton_ID_1    Proton_ID_2    Gamow Factor    Tunneling_Prob    Energy(MeV)")
...
while no_step < (total_steps + 1):
    ...
    for Na in range(num_particles[1]):
        for Nb in range(1, num_particles[1]):
            ...
            if(not(Na == Nb)):
                ...
                cross_sec, prob_coll = fusionProbability.prob_collision([vp_x[Na], vp_y[Na], vp_z[Na]], [vp_x[Nb], vp_y[Nb], vp_z[Nb]], U_pp[Na][Nb])
                print(f"{no_step}    {Na}    {Nb}    {pG}    {tunnel_prob}    {prob_coll}    {cross_sec}    {U1/(e*np.power(10, 6))}    {U2/(e*np.power(10, 6))}")
    ...
    for Np in range(num_particles[1]):
        ...
        for Ne in range(num_particles[0]):
            ...
            v1_x = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/me) - ve_x[Ne])
            v1_y = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/me) - ve_y[Ne])
            v1_z = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/me) - ve_z[Ne])

            v2_x = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/mp) - vp_x[Np])
            v2_y = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/mp) - vp_y[Np])
            v2_z = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/mp) - vp_z[Np])
