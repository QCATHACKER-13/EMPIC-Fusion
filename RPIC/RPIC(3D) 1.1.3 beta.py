...
class Quant_Prob:
    ...
    def prob_tunnel(self, q1, q2, v1, v2, PE):
        ...
        E = KE + PE
        gamma = np.sqrt(np.abs(Eg/E)) - np.sqrt(np.abs(self.r0/R0))
        return KE, gamma
...
while t < (t_max + 1):
    ...
    print("Proton_ID_1    Proton_ID_2    Gamow Factor    Tunneling_Prob")
    for Na in range(num_particles[1]):
        for Nb in range(1, num_particles[1]):
            if(not(Na == Nb)):
                fusionProbability = Quant_Prob(mp, mp, 1, 1)
    ...
    for Na in range(num_particles[1]):
        for Nb in range(num_particles[0]):
            neutronProbability = Quant_Prob(me, mp, 1, 1)
            U1, pG = fusionProbability.gamow_factor([vp_x[Na], vp_y[Na], vp_z[Na]], [ve_x[Nb], ve_y[Nb], ve_z[Nb]], U_pe[Na][Nb])
