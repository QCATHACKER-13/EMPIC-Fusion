...
vx_e = np.zeros(num_particles[0])
vy_e = np.zeros(num_particles[0])
vz_e = np.zeros(num_particles[0])

#initial velocity of proton interaction in otehr particles
vx_p = np.zeros(num_particles[1])
vy_p = np.zeros(num_particles[1])
vz_p = np.zeros(num_particles[1])
...
#-----------------------------LORENTZ FACTOR CHECKER-----------------------------

num_initial = [0.0, 0.0]
while not(num_initial[0] == (3*num_particles[0])):
    N_x = 0.0
    N_y = 0.0
    N_z = 0.0
   while not(N_x == num_particles[0]):
       if((ve_x[N_x]/c0)<c0):
           num_initial[0] += 1
       elif((ve_x[N_x]/c0)>c0):
           ve_x[N_x] = np.random.uniform(-c0, c0, 1)
           N_x -= 1

   while not(N_y == num_particles[0]):
       if((ve_y[N_y]/c0)<c0):
           N_y[0] += 1
       elif((ve_y[N_y]/c0)>c0):
           ve_x[N_y] = np.random.uniform(-c0, c0, 1)
           N_y -= 1

    for N in range(num_particles[0]):
            ...
            num_initial[0] -= 1
            ve_y[N] = np.random.uniform(-c0, c0, 1)
        elif((ve_z[N]/c0)>c0):
            num_initial[0] -= 1
            ve_z[N] = np.random.uniform(-c0, c0, 1)

while not(num_initial[1] == (3*num_particles[1])):
    for N in range(num_particles[1]):
        if((vp_x[N]/c0)<c0):
            ...
            num_initial[1] -= 1
            vp_x[N] = np.random.uniform(-c0, c0, 1)
        elif((vp_y[N]/c0)>c0):
            num_initial[1] -= 1
            vp_y[N] = np.random.uniform(-c0, c0, 1)
        elif((ve_z[N]/c0)>c0):
            num_initial[1] -= 1
            vp_z[N] = np.random.uniform(-c0, c0, 1)


#-----------------------------END LORENTZ FACTOR CHECKER-----------------------------
...
while t < (t_max + 1):
    ...
    for Ne in range(num_particles[0]):
        ...
        z_electron = np.mod(z_electron + a, (l1 - a) - z_electron)
    ...
    for Np in range(num_particles[1]):
        ...
        z_proton = np.mod(z_proton + a, (l1 - a) - z_proton)
    ...
    for Nei in range(num_particles[0]):
        ...
        for Nej in range(1, num_particles[0]):
            ...
            if(not(Nei == Nej)):
                ...
                vx_e[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_x[Nei][Nej]))/me) - ve_x[Nej])
                vy_e[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_y[Nei][Nej]))/me) - ve_y[Nej])
                vy_e[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_z[Nei][Nej]))/me) - ve_z[Nej])
                ...
                electron_particle = Particle_Field(me, -e, [vx_e[Nej], vy_e[Nej], vz_e[Nej]],
    ...
    for Npi in range(num_particles[1]):
        ...
        for Npj in range(1, num_particles[1]):
            ...
            if(not(Npi == Npj)):
                ...
                vx_p[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_x[Npi][Npj]))/mp) - vp_x[Npj])
                vy_p[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_y[Npi][Npj]))/mp) - vp_y[Npj])
                vy_p[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_z[Npi][Npj]))/mp) - vp_z[Npj])
                ...
                proton_particle = Particle_Field(mp, e, [vx_p[Npj], vy_p[Npj], vz_p[Npj]],
    ...
    for Np in range(num_particles[1]):
        ...
        for Ne in range(num_particles[0]):
            ...
            vx_e[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/me) - ve_x[Ne])
            vy_e[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/me) - ve_y[Ne])
            vy_e[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/me) - ve_z[Ne])

            vx_p[Np] = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/mp) - vp_x[Np])
            vy_p[Np] = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/mp) - vp_y[Np])
            vy_p[Np] = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/mp) - vp_z[Np])
            ...
            electron_particle = Particle_Field(me, -e, [vx_e[Ne], vy_e[Ne], vz_e[Ne]],
                                         [Ep_xField[Np], Ep_yField[Np], Ep_zField[Np]],
                                         [Bp_xField[Np], Bp_yField[Np], Bp_zField[Np]],
                                         dt)
            proton_particle = Particle_Field(mp, e, [vx_p[Np], vy_p[Np], vz_p[Np]],
