...
class Field:
    ...
    #Charge Grid
    def Q_grid(self, Vab, x):
        #Charge from Electric Potential
        qx = (Vab*np.power(self.r, 2))/(k0*self.di)
        qy = (Vab*np.power(self.r, 2))/(k0*self.dj)
        qz = (Vab*np.power(self.r, 2))/(k0*self.dk)
...
class Particle:
    ...
    def acceleration(self):
        ...
        a0_x = ((self.q/self.m)*((self.Ex + (((self.vy*self.Bz) - ((self.vz*self.By)*np.power(self.gamma, 3)))))))
        a0_y = ((self.q/self.m)*((self.Ey + (((self.vz*self.Bx) - ((self.vx*self.Bz)*np.power(self.gamma, 3)))))))
        a0_z = ((self.q/self.m)*((self.Ez + (((self.vx*self.By) - ((self.vy*self.Bx)*np.power(self.gamma, 3)))))))

        a_x = a0_x*np.sqrt(np.power(1 - np.power(self.v/self.c, 2),3))
        a_y = a0_y*np.sqrt(np.power(1 - np.power(self.v/self.c, 2),3))
        a_z = a0_z*np.sqrt(np.power(1 - np.power(self.v/self.c, 2),3))
    ...
    def velocity(self, a_particle):
        ...
        v_x = (self.vx + (a_particle[0]*self.t))/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2)))
        v_y = (self.vy + (a_particle[1]*self.t))/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2)))
        v_z = (self.vz + (a_particle[2]*self.t))/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2)))
        return v_x, v_y, v_z

    def position(self, x, v):
        #position of the particle
        position_x = x[0] + ((v[0]*self.t)/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2))))
        position_y = x[1] + ((v[1]*self.t)/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2))))
        position_z = x[2] + ((v[2]*self.t)/np.sqrt(1 - ((self.v*a_resultant*dt)*np.power(self.v/self.c, 2))))
...
while t < t_max:
    ...
    for Ne in range(num_particles[0]):
        ...
        Q_xa, Q_ya, Q_za = Sa_electron_field.Q_grid(Q, [x_electron[Ne], y_electron[Ne], z_electron[Ne]])
        Q_xb, Q_yb, Q_zb = Sb_electron_field.Q_grid(-Q, [x_electron[Ne], y_electron[Ne], z_electron[Ne]])

        Qx[0][Ne] = (0.5*(Q_xa - Q_xb))
        Qy[0][Ne] = (0.5*(Q_ya - Q_yb))
        Qz[0][Ne] = (0.5*(Q_za - Q_zb))
        ...
        Uqex1, Uqey1, Uqez1 = Sa_electron_field.U([-e, -e, -e], [Qx[0][Ne], Qy[0][Ne], Qz[0][Ne]])
        Uqex2, Uqey2, Uqez2 = Sb_electron_field.U([-e, -e, -e], [Qx[0][Ne], Qy[0][Ne], Qz[0][Ne]])
        ...
        v = np.sqrt(np.power(ve_x[Ne],2) + np.power(ve_y[Ne], 2) + np.power(ve_z[Ne], 2))
        ...
        #ve_y[Ne] = (0.5*((np.sqrt((2*np.abs(Uqey1))/me)) + (np.sqrt((2*np.abs(Uqey2))/me)))) - ve_x[Ne]
        ...
        gamma = 1/np.sqrt(1 - np.power(v/c0,2))
        
        #acceleration of the particle in electrostatic force between electrode surface area and charged particle
        x_acceleration = ((-e/me)*(Ex_field[0][Ne] + (0.5*((ve_y[Ne]*Bz_field[0][Ne]) - (ve_z[Ne]*By_field[0][Ne])))))/np.power(gamma, 3)
        y_acceleration = ((-e/me)*(Ey_field[0][Ne] + (0.5*((ve_z[Ne]*Bx_field[0][Ne]) - (ve_x[Ne]*Bz_field[0][Ne])))))/np.power(gamma, 3)
        z_acceleration = ((-e/me)*(Ez_field[0][Ne] + (0.5*((ve_x[Ne]*By_field[0][Ne]) - (ve_y[Ne]*Bx_field[0][Ne])))))/np.power(gamma, 3)

        acceleration = np.sqrt(np.power(x_acceleration,2) + np.power(y_acceleration, 2) + np.power(z_acceleration, 2))
        gamma_a = 1/np.sqrt(1 - ((v*acceleration*dt)/np.power(c0,2)))
        
        vx_electron = (ve_x[Ne] + (0.5*x_acceleration*dt))*gamma_a
        vy_electron = (ve_y[Ne] + (0.5*y_acceleration*dt))*gamma_a
        vz_electron = (ve_z[Ne] + (0.5*z_acceleration*dt))*gamma_a
        print(f"velocity: {v/c0}, {ve_x[Ne]}, {ve_y[Ne]}, {ve_z[Ne]}")

        #print(f"acceleration{x_acceleration, y_acceleration, z_acceleration}")
        #print(f"gamma: {gamma, gamma_a}")
        #print(f"velocity: {v/c0}, {vx_electron}, {vx_electron}, {vx_electron}")
