...
while t < (t_max + 1):
    ...
    for Nei in range(num_particles[0]):
        ...
        for Nej in range(1, num_particles[0]):
            ...
            if(not(Nei == Nej)):
                ...
                ve_x[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_x[Nei][Nej]))/me) - ve_x[Nej])
                ve_y[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_y[Nei][Nej]))/me) - ve_y[Nej])
                ve_z[Nej] = 0.5*(np.sqrt((2*np.abs(Uee_z[Nei][Nej]))/me) - ve_z[Nej])
    ...
    for Npi in range(num_particles[1]):
        ...
        for Npj in range(1, num_particles[1]):
            ...
            if(not(Npi == Npj)):
                ...
                vp_x[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_x[Npi][Npj]))/mp) - vp_x[Npj])
                vp_y[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_y[Npi][Npj]))/mp) - vp_y[Npj])
                vp_z[Npj] = 0.5*(np.sqrt((2*np.abs(Upp_z[Npi][Npj]))/mp) - vp_z[Npj])
    ...
    for Np in range(num_particles[1]):
        ...
        for Ne in range(num_particles[0]):
            ...
            ve_x[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/me) - ve_x[Ne])
            ve_y[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/me) - ve_y[Ne])
            ve_z[Ne] = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/me) - ve_z[Ne])

            vp_x[Np] = 0.5*(np.sqrt((2*np.abs(Upe_x[Np][Ne]))/mp) - vp_x[Np])
            vp_y[Np] = 0.5*(np.sqrt((2*np.abs(Upe_y[Np][Ne]))/mp) - vp_y[Np])
            vp_z[Np] = 0.5*(np.sqrt((2*np.abs(Upe_z[Np][Ne]))/mp) - vp_z[Np])
