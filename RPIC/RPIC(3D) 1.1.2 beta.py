...
alpha = k0*(np.power(e,2)/(h_bar*c0)) #fine constant
r0 = 1.25e-15 #nuclear radius parameter
...
class Field:
    ...
    def __init__(self, xi, xf):
        ...
        #radial coordinate
        self.rho = np.sqrt(np.power(self.x - self.x0,2) + np.power(self.y - self.y0,2))
        self.r = np.sqrt(np.power(self.x - self.x0,2) + np.power(self.y - self.y0,2) + np.power(self.z - self.z0,2))

        #angular coordinate
        self.theta = np.arctan((xf[1] - xi[1])/(xf[0] - xi[0]))
...
class Particle_Field:
    ...
    def Maxwell_BoltzmannSD(self, m1, m2, v1, v2):
        lx = h/((m1*v1[0]) + (m2*v2[0]))
        ly = h/((m1*v1[1]) + (m2*v2[1]))
        lz = h/((m1*v2[2]) + (m2*v2[2]))
        return lx, ly, lz
#--------------------------END PARTICLE-------------------------
...
class Nuke_Prob:
    ...
    def prob_tunnel(self, vx, vy, vz):

        v = np.sqrt(np.power(vx, 2) + np.power(vy, 2) + np.power(vz, 2))
        reduce_mass = (self.m1*self.m2)/(self.m1 + self.m2)
        E0 = np.power((k0*np.power(e,2))*((np.pi*np.sqrt(2*reduce_mass))/(2*h_bar)),2)
        R0 = 1/np.power(2*np.sqrt(k0*np.power(e,2))*(np.sqrt(2*reduce_mass)/h_bar),2)
        E = 0.5*reduce_mass*np.power(v, 2)
        prob = np.sqrt(E0/E) - np.sqrt(r0/R0)
        return E0, prob
...
while t < (t_max + 1):
    ...
    for Na in range(num_particles[1]):
        for Nb in range(num_particles[1]):
            if(not(Na == Nb)):
                fusionProbability = Probablity(mp, mp)
                U, pG = fusionProbability.gamow_factor(2, [vp[0][Na], vp[1][Na], vp[2][Na]], [vp[0][Nb], vp[1][Nb], vp[2][Nb]], [0,0,0], [0,0,0])
    ...
    for Na in range(num_particles[1]):
        for Nb in range(num_particles[0]):
            neutronProbability = Probablity(me, mp)
            U, pG = fusionProbability.gamow_factor(2, [vp[0][Na], vp[1][Na], vp[2][Na]], [ve[0][Nb], ve[1][Nb], ve[2][Nb]], [0,0,0], [0,0,0])
            print(f"{Na}    {Nb}    {pG}    {(U/e)*1e-6}")
    ...
    for Nei in range(num_particles[0]):
        ...
        for Nej in range(1, num_particles[0]):
            ...
            if(not(Nei == Nej)):
                ...
                electron_particle = Particle_Field(me, [ex, ey, ez], [vx_e[Nej], vy_e[Nej], vz_e[Nej]],
    ...
    for Npi in range(num_particles[1]):
        ...
        for Npj in range(1, num_particles[1]):
            ...
            if(not(Npi == Npj)):
                ...
                proton_particle = Particle_Field(mp, [ex, ey, ez], [vx_p[Npj], vy_p[Npj], vz_p[Npj]],
    ...
    for Np in range(num_particles[1]):
        ...
        for Ne in range(num_particles[0]):
            ...
            electron_particle = Particle_Field(me, [ex, ey, ez], [vx_e[Ne], vy_e[Ne], vz_e[Ne]],
                                         [Ep_xField[Np], Ep_yField[Np], Ep_zField[Np]],
                                         [Bp_xField[Np], Bp_yField[Np], Bp_zField[Np]],
                                         dt)
            proton_particle = Particle_Field(mp, [px, py, pz], [vx_p[Np], vy_p[Np], vz_p[Np]],
