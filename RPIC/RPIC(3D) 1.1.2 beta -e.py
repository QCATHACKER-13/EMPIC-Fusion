...
alpha = k0*(np.power(e,2)/(h_bar*c0))
...
class Field:
    ...
    #Electric Potential
    def V(self, q):
        V_x = (k0*q[0]*(self.x - self.x0))/np.power(self.r,2)
        V_y = (k0*q[1]*(self.y - self.y0))/np.power(self.r,2)
        V_z = (k0*q[2]*(self.z - self.z0))/np.power(self.r,2)
        return V_x, V_y, V_z
...
class Particle_Field:
    ...
    def acceleration(self):
        ...
        a0_x = ((self.qx/self.m)*((self.Ex + (((self.vy*self.Bz) - ((self.vz*self.By)))))))
        a0_y = ((self.qy/self.m)*((self.Ey + (((self.vz*self.Bx) - ((self.vx*self.Bz)))))))
        a0_z = ((self.qz/self.m)*((self.Ez + (((self.vx*self.By) - ((self.vy*self.Bx)))))))
...
class Probablity:
    ...
    def gamow_factor(self, n, v1, v2, v3, v4):
        v_x = (v1[0] + v2[0] + v3[0] + v4[0])/n
        v_y = (v1[1] + v2[1] + v3[1] + v4[1])/n
        v_z = (v1[2] + v2[2] + v3[2] + v4[2])/n
        
        reduce_mass = (self.m1*self.m2)/(self.m1 + self.m2)
        
        c = np.sqrt(np.power(v_x,2) + np.power(v_y,2) + np.power(v_z,2))
        
        energy = np.abs(0.5*reduce_mass*np.power(c, 2))
        Eg = 2*reduce_mass*np.power(self.c,2)*np.power(np.pi*alpha*self.Z1*self.Z2, 2)
        
        gamow = np.exp(-np.sqrt(np.abs(Eg/energy)))
        return energy, gamow
...
while t < (t_max + 1):
    ...
    for Ne in range(num_particles[0]):
        ...
        Vqex1, Vqey1, Vqez1 = Sa_electron_field.V([Q_xa, Q_ya, Q_za])
        Vqex2, Vqey2, Vqez2 = Sb_electron_field.V([Q_xb, Q_yb, Q_zb])

        Uqe_x[Ne] = ((-e*Vqex1) - (-e*Vqex2))
        Uqe_y[Ne] = ((-e*Vqey1) - (-e*Vqey2))
        Uqe_z[Ne] = ((-e*Vqez1) - (-e*Vqez2))
        U_qe[Ne] = np.sqrt(np.power(((-e*Vqex1) - (-e*Vqex2)),2) + np.power(((-e*Vqey1) - (-e*Vqey2)),2) + np.power(((-e*Vqez1) - (-e*Vqez2)),2))

        ve_x[Ne] = (np.sqrt((2*np.abs(((-e*Vqex1) - (-e*Vqex2))))/me) - ve_x[Ne])
        ve_y[Ne] = (np.sqrt((2*np.abs(((-e*Vqey1) - (-e*Vqey2))))/me) - ve_y[Ne])
        ve_z[Ne] = (np.sqrt((2*np.abs(((-e*Vqez1) - (-e*Vqez2))))/me) - ve_z[Ne])

        e_xa, e_ya, e_za = Sa_electron_field.e_field(-e)
        e_xb, e_yb, e_zb = Sb_electron_field.e_field(-e)

        ex = (0.5*(e_xa + e_xb))
        ey = (0.5*(e_ya + e_yb))
        ez = (0.5*(e_za + e_zb))

        electron_particle = Particle_Field(me, [ex, ey, ez], [ve_x[Ne], ve_y[Ne], ve_z[Ne]],
    ...
    for Np in range(num_particles[1]):
        ...
        Vqpx1, Vqpy1, Vqpz1 = Sa_proton_field.V([Q_xa, Q_ya, Q_za])
        Vqpx2, Vqpy2, Vqpz2 = Sa_proton_field.V([Q_xb, Q_yb, Q_zb])

        Uqp_x[Np] = ((e*Vqpx1) - (e*Vqpx2))
        Uqp_y[Np] = ((e*Vqpy1) - (e*Vqpy2))
        Uqp_z[Np] = ((e*Vqpz1) - (e*Vqpz2))
        U_qp[Np] = np.sqrt(np.power(((e*Vqpx1) - (e*Vqpx2)),2) + np.power(((e*Vqpy1) - (e*Vqpy2)),2) + np.power(((e*Vqpz1) - (e*Vqpz2)),2))

        vp_x[Np] = (np.sqrt((2*np.abs(((e*Vqpx1) - (e*Vqpx2))))/mp) - vp_x[Np])
        vp_y[Np] = (np.sqrt((2*np.abs(((e*Vqpy1) - (e*Vqpy2))))/mp) - vp_y[Np])
        vp_z[Np] = (np.sqrt((2*np.abs(((e*Vqpz1) - (e*Vqpz2))))/mp) - vp_z[Np])

        e_xa, e_ya, e_za = Sa_proton_field.e_field(e)
        e_xb, e_yb, e_zb = Sb_proton_field.e_field(e)

        ex = (0.5*(e_xa + e_xb))
        ey = (0.5*(e_ya + e_yb))
        ez = (0.5*(e_za + e_zb))

        proton_particle = Particle_Field(mp, [ex, ey, ez], [vp_x[Np], vp_y[Np], vp_z[Np]],
    ...
    print("Proton_ID_1    Proton_ID_2    Probability    Energy(MeV)")
    for Na in range(num_particles[1]):
        for Nb in range(num_particles[1]):
            if(not(Na == Nb)):
                fusionProbability = Probablity(mp, mp, 1, 1)
                U, pG = fusionProbability.gamow_factor(2, [vp_x[Na], vp_y[Na], vp_z[Na]], [vp_x[Nb], vp_y[Nb], vp_z[Nb]], [0,0,0], [0,0,0])
                #print(f"{Na}    {Nb}    {pG}    {(U/e)*1e-6}")

    print()
    print("Probability of Neutron Presence")
    print("Proton_ID_1    Electron_ID_2    Probability    Energy(MeV)")
    for Na in range(num_particles[1]):
        for Nb in range(num_particles[0]):
            neutronProbability = Probablity(me, mp, 1, 1)
            U, pG = fusionProbability.gamow_factor(2, [vp_x[Np], vp_y[Np], vp_z[Np]], [ve_x[Nb], ve_y[Nb], ve_z[Nb]], [0,0,0], [0,0,0])
            print(f"{Na}    {Nb}    {pG}    {(U/e)*1e-6}     {[vp_x[Np], vp_y[Np], vp_z[Np]]}    {[ve_x[Nb], ve_y[Nb], ve_z[Nb]]}")
    #-----------------------------END Gamow Factor Prob-----------------------------
    ...
    for Nei in range(num_particles[0]):
        ...
        for Nej in range(1, num_particles[0]):
            ...
            if(not(Nei == Nej)):
                ...
                ve_x[Nej] = (np.sqrt((2*np.abs(Uee_x[Nei][Nej]))/me) - ve_x[Nej])
                ve_y[Nej] = (np.sqrt((2*np.abs(Uee_y[Nei][Nej]))/me) - ve_y[Nej])
                ve_z[Nej] = (np.sqrt((2*np.abs(Uee_z[Nei][Nej]))/me) - ve_z[Nej])

                Eee_x, Eee_y, Eee_z = ee_interaction.Ep(-e)
                Bee_x, Bee_y, Bee_z = ee_interaction.Bp(-e, [ve_x[Nej], ve_y[Nej], ve_z[Nej]])
                ...
                e_xa, e_ya, e_za = ee_interaction.e_field(-e)
                e_xb, e_yb, e_zb = ee_interaction.e_field(-e)

                ex = (0.5*(e_xa + e_xb))
                ey = (0.5*(e_ya + e_yb))
                ez = (0.5*(e_za + e_zb))

                electron_particle = Particle_Field(me, [ex, ey, ez], [ve_x[Nej], ve_y[Nej], ve_z[Nej]],
    ...
    for Npi in range(num_particles[1]):
        ...
        for Npj in range(1, num_particles[1]):
            ...
            if(not(Npi == Npj)):
                ...
                KE = np.sqrt(np.power(Uppx,2) + np.power(Uppy,2) + np.power(Uppz,2))
                ...
                vp_x[Npj] = (np.sqrt((2*np.abs(Upp_x[Npi][Npj]))/mp) - vp_x[Npj])
                vp_y[Npj] = (np.sqrt((2*np.abs(Upp_y[Npi][Npj]))/mp) - vp_y[Npj])
                vp_z[Npj] = (np.sqrt((2*np.abs(Upp_z[Npi][Npj]))/mp) - vp_z[Npj])

                Epp_x, Epp_y, Epp_z = pp_interaction.Ep(e)
                Bpp_x, Bpp_y, Bpp_z = pp_interaction.Bp(e, [vp_x[Npj], vp_y[Npj], vp_z[Npj]])
                ...
                e_xa, e_ya, e_za = pp_interaction.e_field(e)
                e_xb, e_yb, e_zb = pp_interaction.e_field(e)

                ex = (0.5*(e_xa + e_xb))
                ey = (0.5*(e_ya + e_yb))
                ez = (0.5*(e_za + e_zb))

                proton_particle = Particle_Field(mp, [ex, ey, ez], [vp_x[Npj], vp_y[Npj], vp_z[Npj]],
    ...
    for Np in range(num_particles[1]):
        ...
        for Ne in range(num_particles[0]):
            ...
            KE = np.sqrt(np.power(Upex,2) + np.power(Upey,2) + np.power(Upez,2))
            ...
            ve_x[Ne] = (np.sqrt((2*np.abs(Upe_x[Np][Ne]))/me) - ve_x[Ne])
            ve_y[Ne] = (np.sqrt((2*np.abs(Upe_y[Np][Ne]))/me) - ve_y[Ne])
            ve_z[Ne] = (np.sqrt((2*np.abs(Upe_z[Np][Ne]))/me) - ve_z[Ne])

            vp_x[Np] = (np.sqrt((2*np.abs(Upe_x[Np][Ne]))/mp) - vp_x[Np])
            vp_y[Np] = (np.sqrt((2*np.abs(Upe_y[Np][Ne]))/mp) - vp_y[Np])
            vp_z[Np] = (np.sqrt((2*np.abs(Upe_z[Np][Ne]))/mp) - vp_z[Np])

            Ee_x, Ee_y, Ee_z = pe_interaction.Ep(-e)
            Be_x, Be_y, Be_z = pe_interaction.Bp(-e, [ve_x[Ne], ve_y[Ne], ve_z[Ne]])
            
            Ep_x, Ep_y, Ep_z = pe_interaction.Ep(e)
            Bp_x, Bp_y, Bp_z = pe_interaction.Bp(e, [vp_x[Np], vp_y[Np], vp_z[Np]])
            ...
            e_xa, e_ya, e_za = pe_interaction.e_field(-e)
            e_xb, e_yb, e_zb = pe_interaction.e_field(-e)

            p_xa, p_ya, p_za = pe_interaction.e_field(e)
            p_xb, p_yb, p_zb = pe_interaction.e_field(e)
            ...
            electron_particle = Particle_Field(me, [ex, ey, ez], [ve_x[Ne], ve_y[Ne], ve_z[Ne]],
                                         ...
                                         dt)
            proton_particle = Particle_Field(mp, [px, py, pz], [vp_x[Np], vp_y[Np], vp_z[Np]],
