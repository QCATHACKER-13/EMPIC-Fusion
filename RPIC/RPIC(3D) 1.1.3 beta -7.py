...
class Quant_Prob:
    ...
    def gamow_factor(self, v1, v2, U):
        ...
        E1g = 2*self.mu*np.power(self.c,2)*np.power(np.pi*self.alpha*self.Z1*self.Z2, 2)
    ...
    def prob_collision(self, N, v1, v2, U):
        ...
        return col_csec, prob_col, prob_fus
...
print("Step    Proton_ID_1    Proton_ID_2    Gamow Factor    Proton's Energy(MeV)    Detected Energy(MeV)")
    ...
    for Na in range(num_particles[1]):
        ...
        for Nb in range(1, num_particles[1]):
            ...
            if(not(Na == Nb)):
                ...
                cross_sec, prob_coll, prob_fuss = fusionProbability.prob_collision(num_particles[1], [vp_x[Na], vp_y[Na], vp_z[Na]], [vp_x[Nb], vp_y[Nb], vp_z[Nb]], U_pp[Na][Nb])
                ...
                #print(f"{no_step}    {listing_no}    {Na}    {Nb}    {fusion_no}    {collision_no}    {rc}    {pG}    {tunnel_prob}    {prob_coll + (1 - tunnel_prob)}    {cross_sec}    {U2/(e*np.power(10, 6))}    {(U_pp[Na][Nb]/(e*np.power(10, 6)))}    {(U2/(e*np.power(10, 6))) - (U_pp[Na][Nb]/(e*np.power(10, 6)))}")
