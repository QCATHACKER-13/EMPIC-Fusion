...
class Particle_Field:
    ...
    def gamow_factor(self, m1, m2, Z1, Z2, v1, v2):
        reduce_mass = (m1*m2)/(m1 + m2)
        c = np.sqrt(np.power(0.5*(v1[0] + v2[0]),2) + np.power(0.5*(v1[1] + v2[1]),2) + np.power(0.5*(v1[2] + v2[2]),2))
        E = 0.5*reduce_mass*np.power(c, 2)
        Eg = 2*reduce_mass*np.power(self.c,2)*np.power(np.pi*alpha*Z1*Z2)
...
while t < (t_max + 1):
    ...
    print("")
    print("for ee_interaction: Particle_1  Particle_2  Energy(J)")
    ...
    for Nei in range(num_particles[0]):
        ...
        for Nej in range(1, num_particles[0]):
            ...
            if(not(Nei == Nej)):
                ...
                print(f"{Nei} {Nej} {U_ee[Nei][Nej]}")

            else:
                break
    print("")
    print("for pp_interaction: Particle_1  Particle_2  Energy(J)")
    ...
    for Npi in range(num_particles[1]):
        ...
        for Npj in range(1, num_particles[1]):
            ...
            if(not(Npi == Npj)):
                ...
                print(f"{Npi} {Npj} {U_pp[Npi][Npj]}")

            else:
                break

    print("")
    print("for pe_interaction: Particle_1  Particle_2  Energy(J)")
