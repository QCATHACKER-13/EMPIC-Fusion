...
class Particle:
    ...
    def acceleration(self):
        ...
        a_x = ((self.q/(self.m*self.c))*(((self.Ex)/self.gamma) + (((((self.vy - self.v0_y)*self.Bz)/self.gamma) - (((self.vz - self.v0_z)*self.By)/self.gamma)))))
        a_y = ((self.q/(self.m*self.c))*(((self.Ey)/self.gamma) + (((((self.vz - self.v0_z)*self.Bx)/self.gamma) - (((self.vx - self.v0_x)*self.Bz)/self.gamma)))))
        a_z = ((self.q/(self.m*self.c))*(((self.Ez)/self.gamma) + (((((self.vx - self.v0_x)*self.By)/self.gamma) - (((self.vy - self.v0_y)*self.Bx)/self.gamma)))))
        return a_x, a_y, a_z
    ...
    def velocity(self, a_particle):
        ...
        v_x = (((self.vx - self.v0_x) + (0.5*a_particle[0]*self.t))*np.sqrt(1 - np.power((self.v + (a_resultant*self.t))/self.c, 2)))
        v_y = (((self.vy - self.v0_y) + (0.5*a_particle[1]*self.t))*np.sqrt(1 - np.power((self.v + (a_resultant*self.t))/self.c, 2)))
        v_z = (((self.vz - self.v0_z) + (0.5*a_particle[2]*self.t))*np.sqrt(1 - np.power((self.v + (a_resultant*self.t))/self.c, 2)))
        return v_x, v_y, v_z
...
for step in range(steps):
    ...
    for Ne in range(num_particles[0]):
        ...
        ve_x[Ne] -= (((0.5*(w1_x1 + w2_x1))*np.sqrt((2*np.abs(Uqex2))/me)) - ((0.5*(w1_x2 + w2_x2))*np.sqrt((2*np.abs(Uqex1))/me)))
        ve_y[Ne] -= (((0.5*(w1_y1 + w2_y1))*np.sqrt((2*np.abs(Uqey2))/me)) - ((0.5*(w1_y2 + w2_y2))*np.sqrt((2*np.abs(Uqey1))/me)))
        ve_z[Ne] -= (((0.5*(w1_z1 + w2_z1))*np.sqrt((2*np.abs(Uqez2))/me)) - ((0.5*(w1_z2 + w2_z2))*np.sqrt((2*np.abs(Uqez1))/me)))
