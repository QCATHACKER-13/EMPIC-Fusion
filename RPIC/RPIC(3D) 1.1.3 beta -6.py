    ...
    for Na in range(num_particles[1]):
        ...
        for Nb in range(1, num_particles[1]):
            ...
            if(not(Na == Nb)):
                ...
                cross_sec, prob_coll = fusionProbability.prob_collision(num_particles[1], [vp_x[Na], vp_y[Na], vp_z[Na]], [vp_x[Nb], vp_y[Nb], vp_z[Nb]], U_pp[Na][Nb])
                print(f"{no_step}    {listing_no}    {Na}    {Nb}    {fusion_no}    {collision_no}    {rc}    {pG}    {tunnel_prob}    {prob_coll}    {cross_sec}    {U2/(e*np.power(10, 6))}    {(Up - U2)/(e*np.power(10, 6))}")
