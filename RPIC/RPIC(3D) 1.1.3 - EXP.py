...
Energy_Input = 1667.697659 #keV unit
#Electric potential of particle's interaction between them
Uqe_x = np.full(num_particles[0], (Energy_Input*1e3)/e)
Uqe_y = np.full(num_particles[0], (Energy_Input*1e3)/e)
Uqe_z = np.full(num_particles[0], (Energy_Input*1e3)/e)
U_qe = np.full(num_particles[0], (Energy_Input*1e3)/e)

Uqp_x = np.full(num_particles[1], (Energy_Input*1e3)/e)
Uqp_y = np.full(num_particles[1], (Energy_Input*1e3)/e)
Uqp_z = np.full(num_particles[1], (Energy_Input*1e3)/e)
U_qp = np.full(num_particles[1], (Energy_Input*1e3)/e)
...
class Quant_Prob:
    ...
    def nuclear_radius(self, v1, v2):
        ...
        rc = (2*k0*np.power(self.e, 2))/(self.reduce_mass*np.power(v_r, 2))
        return rc
