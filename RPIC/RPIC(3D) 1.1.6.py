...
class Quant_Prob:
    ...
    def prob_tunnel(self, q1, q2, v1, v2, U):
        ...
        #E = 0.5*self.reduce_mass*np.power(v_r,2) + U
        #Eg = 2*self.mu*np.power(self.c,2)*np.power(np.pi*self.alpha*self.Z1*self.Z2, 2)
        E = 0.5*self.reduce_mass*np.power(v_cm,2) + U
...
print("Step,           Proton_ID_1,           Proton_ID_2,           Fusion_No,           Collision_No,           Tunnel_Prob,           Prob_Collision,           Prob_Fusion,           Radius(Distance_PP),           Cross Section,           Maxwell_Boltz_Dis,           Velocity_CM_PP,           Rela_V_CM_PP,           PP's KE(MeV),           PE(MeV)")
...
while no_step < (total_steps + 1):
    ...
    for Ne in range(num_particles[0]):
        ...
        B2_x1, B2_y1, B2_z1 = Sa_electron_external_MagneticField.B2(I_external)
        B2_x2, B2_y2, B2_z2 = Sb_electron_external_MagneticField.B2(I_external)
    ...
    for Np in range(num_particles[1]):
        ...
        B2_x1, B2_y1, B2_z1 = Sa_proton_external_MagneticField.B2(I_external)
        B2_x2, B2_y2, B2_z2 = Sb_proton_external_MagneticField.B2(I_external)
    ...
    for Na in range(num_particles[1]):
        ...
        for Nb in range(1, num_particles[1]):
            ...
            if(not(Na == Nb)):
                ...
                E1, tunnel_prob = fusionProbability.prob_tunnel(e, e, [vp_x[Na], vp_y[Na], vp_z[Na]], [vp_x[Nb], vp_y[Nb], vp_z[Nb]], U_pp[Na][Nb])
                ...
                if(np.abs(prob_fuss) > prob_coll):
                    fusion_no += 1
                elif(prob_fuss < prob_coll):
                    collision_no += 1

                print(f"{no_step},           {listing_no},           {Na},           {Nb},           {fusion_no},           {collision_no},           {tunnel_prob},           {prob_coll},           {prob_fuss},           {np.abs(tunnel_prob - prob_fuss)},           {rc},           {cross_sec},           {MB_dis} ,          {v_cm},           {v_r},           {E1/(e*np.power(10, 6))},           {(U_pp[Na][Nb]/(e*np.power(10, 6)))}")
