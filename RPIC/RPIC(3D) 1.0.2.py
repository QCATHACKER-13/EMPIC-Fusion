...
for step in range(steps):
    ...
    for Ne in range(num_particles[0]):
        ...
        ve_x2 = (((0.5*(w1_x1 + w2_x1))*np.sqrt((2*np.abs(Uqex2))/me)) - ((0.5*(w1_x2 + w2_x2))*np.sqrt((2*np.abs(Uqex1))/me)))
        ve_y2 = (((0.5*(w1_y1 + w2_y1))*np.sqrt((2*np.abs(Uqey2))/me)) - ((0.5*(w1_y2 + w2_y2))*np.sqrt((2*np.abs(Uqey1))/me)))
        ve_z2 = (((0.5*(w1_z1 + w2_z1))*np.sqrt((2*np.abs(Uqez2))/me)) - ((0.5*(w1_z2 + w2_z2))*np.sqrt((2*np.abs(Uqez1))/me)))
