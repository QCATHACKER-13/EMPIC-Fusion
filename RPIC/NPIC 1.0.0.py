...
class Particle:
    ...
    def acceleration(self):
        ...
        a_x = ((self.q/self.m)*((self.Ex + (((self.vy*self.Bz) - ((self.vz*self.By)))))))
        a_y = ((self.q/self.m)*((self.Ey + (((self.vz*self.Bx) - ((self.vx*self.Bz)))))))
        a_z = ((self.q/self.m)*((self.Ez + (((self.vx*self.By) - ((self.vy*self.Bx)))))))
    ...
    def velocity(self, a_particle):
        ...
        v_x = self.vx + (0.5*a_particle[0]*self.t)
        v_y = self.vy + (0.5*a_particle[1]*self.t)
        v_z = self.vz + (0.5*a_particle[2]*self.t)
        return v_x, v_y, v_z

    def position(self, x):
        #position of the particle
        position_x = x[0] + (0.5*self.vx*self.t)
        position_y = x[1] + (0.5*self.vy*self.t)
        position_z = x[2] + (0.5*self.vz*self.t)
