...
ve_x = np.random.uniform(0, c0, num_particles[0])
ve_y = np.random.uniform(0, c0, num_particles[0])
ve_z = np.random.uniform(0, c0, num_particles[0])

#initial velocity of proton
vp_x = np.random.uniform(0, c0, num_particles[1])
vp_y = np.random.uniform(0, c0, num_particles[1])
vp_z = np.random.uniform(0, c0, num_particles[1])
...
while no_step < (total_steps + 1):
    ...
    for Np in range(num_particles[1]):
        ...
        vp_x[Np] = (np.sqrt((2*np.abs(Uqpx1 - Uqpx2))/mp) - vp_x[Np])
        vp_y[Np] = (np.sqrt((2*np.abs(Uqpy1 - Uqpy2))/mp) - vp_y[Np])
        vp_z[Np] = (np.sqrt((2*np.abs(Uqpz1 - Uqpz2))/mp) - vp_z[Np])
