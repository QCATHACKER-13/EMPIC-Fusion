...
class Particle:
    ...
    def velocity(self, a_particle):
        ...
        v_x = self.vx + ((a_particle[0]*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
        v_y = self.vy + ((a_particle[1]*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
        v_z = self.vz + ((a_particle[2]*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
        return v_x, v_y, v_z

    def position(self, x):
        #position of the particle
        position_x = x[0] + ((self.vx*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
        position_y = x[1] + ((self.vy*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
        position_z = x[2] + ((self.vz*self.t)/np.sqrt(1 - np.power(self.v/self.c, 2)))
